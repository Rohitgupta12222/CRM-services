...
var routes = function () {
    ...
    router.route('/GetAlldata')
        .get(function (req, res) {
            var querytext = 'SELECT "Get_Dashboard_ExpandData"(:p_ref); FETCH ALL IN "abc"';
            var param = {
                replacements: {
            ...
            connect.sequelize
                ...
                .then(function (result) {
                    result.shift();
                    res.status(200).json({ Success: true, Message: "GetAlldata Data Access", Data: result });
                }, function (err) {
                    dataconn.errorlogger('HomeService', 'GetAlldata', err);
                    res.status(200).json({ Success: false, Message: 'User has no access of GetAlldata', Data: null });
                });
        });
    return router;
